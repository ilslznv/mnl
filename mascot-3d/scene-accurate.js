...
import { createVolumeBuilder } from './volume.js?v=4';
...
referenceMaterial.onBeforeCompile = shader => {
  ...
  shader.uniforms.edgePaddingMap = { value: referenceMaterial.userData.edgePadding };
  ...
  shader.fragmentShader = shader.fragmentShader.replace('#include <common>', `#include <common>
    ...
    uniform sampler2D edgePaddingMap;
  ...
  shader.fragmentShader = shader.fragmentShader.replace('#include <map_fragment>', `
    ...
    if (texel.a < 0.99 && viewTurn > 0.0) {
      vec4 edgeColor = texture2D(edgePaddingMap, vMapUv);
    ...
    float referenceLight = 0.36 + 0.64 * max(dot(normalize(vOriginalNormal), keyDirection), 0.0);
...
const addVolume = createVolumeBuilder({ original, triangulate, worldPoint, frontGeometry, referenceMaterial, model });
...
window.__mascot = {
  ...
  resetView,
  setView(yaw, pitch = 0) { resetView(true); targetYaw = yaw; targetPitch = pitch; model.rotation.set(pitch, yaw, 0); },
